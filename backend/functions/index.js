 ...
 */
exports.createRazorpayOrder = onRequest({ region: "asia-south1", cors: true }, (req, res) => {
  cors(req, res, async () => {
 ...
 */
exports.bookAppointmentWithLock = onRequest({ region: "asia-south1", cors: true }, (req, res) => {
  cors(req, res, async () => {
    if (req.method !== "POST") return res.status(405).json({ error: "Method Not Allowed" });
 ...
 */
exports.updateBedInventory = onRequest({ region: "asia-south1", cors: true }, (req, res) => {
  cors(req, res, async () => {
    if (req.method !== "POST") return res.status(405).json({ error: "Method Not Allowed" });
 ...
 */
exports.deductBloodStock = onRequest({ region: "asia-south1", cors: true }, (req, res) => {
  cors(req, res, async () => {
    if (req.method !== "POST") return res.status(405).json({ error: "Method Not Allowed" });
